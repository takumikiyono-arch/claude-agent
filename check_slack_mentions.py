...
THRESHOLD_SEC = 1 * 60 * 60   # 1 時間以上経過したメンションを対象にする
LOOKBACK_SEC  = 24 * 60 * 60  # 遡る最大範囲（24 時間）
...
def build_reminder_text(items: list[dict]) -> str:
    ...
    lines = [
        f":bell: *未返信のメンションが {len(items)} 件あります*（{threshold_h}時間以上未返信）\n"
    ]
