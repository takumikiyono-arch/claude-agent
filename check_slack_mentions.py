...
ACTIVE_HOURS  = range(8, 20)  # 8:00〜19:59 のみ実行（20以降はスキップ）JST基準
...
def main() -> None:
    now_ts = time.time()
    jst_hour = datetime.fromtimestamp(now_ts, tz=JST).hour
    if jst_hour not in ACTIVE_HOURS:
        print(f"[INFO] 現在JST {jst_hour}時 — 実行時間外（8〜19時のみ）のためスキップ。")
        return
