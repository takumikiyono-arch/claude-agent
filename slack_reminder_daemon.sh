...
# 8:00〜19:00 の間、毎時0分に reminder.py を実行するデーモン。
# バックグラウンド起動例:
#   SLACK_BOT_TOKEN=xoxp-... nohup bash slack_reminder_daemon.sh >> /tmp/reminder.log 2>&1 &
...
while true; do
    ...
    if [ "$HOUR" -ge 8 ] && [ "$HOUR" -le 19 ] && [ "$MINUTE" -eq 0 ]; then
        ...
        python3 "$SCRIPT_DIR/reminder.py" || true
        # 二重実行防止のため70秒待機
        sleep 70
    fi
